...
/**
 * Tests for PlantillasResource.
 */
final class PlantillasResourceTest extends TestCase
{
    private HttpClient & MockObject $httpClient;
    private RequestFactory & MockObject $requestFactory;
    private PlantillasResource $resource;
    ...
    protected function setUp(): void
    {
        ...
        $this->resource = new PlantillasResource(
            $this->httpClient,
            $this->requestFactory,
            $config,
        );
    ...
    {
        $path = __DIR__ . '/../../_files/fixtures/plantillas/' . $name . '.json';
        if (!file_exists($path)) {
    ...
    public function testListReturnsPaginatedResponseWithPlantillas(): void
    {
        $fixture = $this->loadFixture('list_success');
        $mockResponse = $this->createMockResponse($fixture, 200);

        $mockRequest = new HttpRequest('/plantillas', null, HttpMethod::GET);
        ...
        $this->assertInstanceOf(PlantillaResponseDTO::class, $result->data[0]);
        $this->assertSame('default', $result->data[0]->clavePlantilla);
    }
}
